...
const [positions, attrs, wI8, c0, c1, c2, c3] = await Promise.all([
  fetchBin('positions.bin','f32'), fetchBin('1-attrs.bin','u8'),
  fetchBin('2-w.bin','i8'),
  fetchBin('4-e0.bin','u8'), fetchBin('5-e1.bin','u8'),
  fetchBin('6-e2.bin','u8'), fetchBin('7-e3.bin','u8')
]);
const edgeBytes = new Uint8Array(c0.length + c1.length + c2.length + c3.length);
{ let o = 0; for (const c of [c0,c1,c2,c3]) { edgeBytes.set(c, o); o += c.length; } }
const edgesU = new Uint32Array(edgeBytes.buffer);
...
for (let e = 0; e < E; e++) {
  E_pre[e] = edgesU[2*e]; E_post[e] = edgesU[2*e+1]; E_w[e] = wI8[e] / 260;
}
...
const csrPost = new Uint32Array(E), csrW = new Float32Array(E);
{
  ...
  // fan-out normalization: hub neurons spread thinner
  for (let i = 0; i < N; i++) {
    const deg = outStart[i+1] - outStart[i];
    const f = 1 / (1 + deg * 0.06);
...
const ptsMat = new THREE.ShaderMaterial({
  ...
  vertexShader: `
    ...
    void main(){
      ...
      float s = (1.0 + aAct*4.0) * uPx * (300.0 / -mv.z);
  ...
  fragmentShader: `
    ...
    void main(){
      ...
      vec3 col = mix(vC*.5, vec3(1.0), vA*.8);
      gl_FragColor = vec4(col, glow * (.13 + vA*.87));
    }`,
...
let spikes = 0, spikeCountWin = 0, lastHud = 0, activeCount = 0, rateSm = 0;
setP(1, 'brain online.');
...
function step(dt) {
  ...
  if (stim && now < stim.until) {
    const arr = stim.idxArr, s = stim.strength * dt * 5.5;
  ...
  let damp = 1;
  for (let i = 0; i < N; i++) if (next[i] > 0.12) activeCount++;
  if (activeCount > 22000) damp = 22000 / activeCount * 0.55;
  activeCount = 0;
  for (let i = 0; i < N; i++) {
    let v = act[i] + next[i] * damp;
    if (v > 1.2) v = 1.2; else if (v < 0.02) v = 0;
    act[i] = v;
...
function animate() {
  ...
  if (performance.now() - lastHud > 400) {
    ...
    rateSm += (spikes/Math.max(dt,1e-3) - rateSm) * .15;
    document.getElementById('h-spikes').textContent = Math.round(rateSm).toLocaleString();
  }
  controls.update();
...
setTimeout(() => stimulate('vision', PRESETS.vision, 0.8, 1300), 2500);
